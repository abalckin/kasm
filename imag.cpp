 ...
 {if (templ[i]!='#') ost<<templ[i];
  ...
  {switch (templ[++i])
   {case 'd': ost<<ndt[0]; break;
    ...
    case 'S': for (int j=0; j<3; ++j) ost<<char(toupper(months[ndt[1]-1][j])); break;
...
{dmyDate (ndt,aday+rday);
 ...
 if (!!inp) if (rd(inp,n,x,y,nskp)!=0) {cerr<<"Reading error from input file."; exit (0);}
...
{fstream inp;
 ...
 if (r) if (rd(inp,n,x,y,nskp)!=0) {cerr<<"Reading error from input file."; exit (0);}
//cout<<"\ntxt\n"; for (int i=0; i<n; ++i) cout<<setw(4)<<i<<setw(14)<<x[i]-x[0]<<setw(14)<<y[i]-y[0]<<'\n';
 return r;
}
  ...
  {w[lw-2]=' '; w[lw-1]=0;
   ...
   {if (w[31]!=' ') r=4;      // blad wiersza danych
    ...
    {while (k<lw && w[k]==' ') ++k;  // opuszczamy spacje
     ...
     if (s=='-') v=-v;
     if (w[k]!=' ' || k!=psd[j]) r=4; else dg[j]=(v==999999?vUNDEF:real(v)*sdg[j]);
    }
    if (r==0)
